...
function cssApp() {
    return gulp.src("src/assets/style/style.sass")
        .pipe(gulpif(!isProduction, sourcemaps.init()))
...
// Сборка HTML страниц. 
/* ВАЖНО! пересборка происходит только при изменниях в папках "layout" и "pages", при измениях в папке "block" не происходит */
function pagesHtml() {
    return gulp.src("src/pages/**/*.pug")
        .pipe(changed("src/pages/**/*.pug"))
		.pipe(data( ()=> {
...
function rewriteAssets() {
    return gulp.src(['build/assets/**/*.+(css|js)'], {base: './build'})
        .pipe(rev())
        .pipe(gulp.dest('build'))
        .pipe(rev.manifest())
        .pipe(gulp.dest('build/assets'))
...
let build = isProduction ? gulp.series(clean, tasks, rewriteAssets, rewriteHtml, serve) : gulp.series(clean, gulp.parallel(tasks, serve, watch));
